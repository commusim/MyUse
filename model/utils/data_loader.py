...
def load_data(dataset_path, resolution, dataset, pid_num, pid_shuffle, cache=True):
    ...
    if not osp.exists(pid_fname):
        ...
        np.save(pid_fname + '1', pid_list[0])
        np.save(pid_fname + '2', pid_list[1])

    pid_list = [np.load(pid_fname + '1.npy',allow_pickle=True),
                np.load(pid_fname + '2.npy',allow_pickle=True)]
