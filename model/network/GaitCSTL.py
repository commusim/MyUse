...
from .Dynamic import Dynamic, Dynamic_conv
from .visual import *
...
class GaitCSTL_Dynamic(nn.Module):
    """Exclude SSFL of CSTL, Ablation study"""

    def __init__(self, hidden_dim=256, part_num=32, div=16):
        super(GaitCSTL_Dynamic, self).__init__()
        self.hidden_dim = hidden_dim
        ...
        self.conv2d_3 = SetBlock(BasicConv2d(_channels[1], _channels[2], 3, padding=1))
        self.conv2d_4 = SetBlock(BasicConv2d(_channels[2], _channels[2], 3, padding=1))
        self.Dynamic = Dynamic(128, 32, 11, "mix")
    ...
    def forward(self, silho):
        ...
        x = self.conv2d_3(x)
        x = self.conv2d_4(x)
        weight = self.Dynamic(x).unsqueeze(1).unsqueeze(2)
        visual_feature(weight, "2d")
        visual_sperate(weight)
