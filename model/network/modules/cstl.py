...
"""     CSTL_modules
modified from CSTL
used CSTL frame,short,long three level to get the better understanding
of the temporal information

but cstl only to get the feature, meaning the h*w in one dimension
"""
...
class ATA(nn.Module):
    ...
    def forward(self, t_f, t_s, t_l):
        ...
        t = t.permute(0, 3, 4, 2, 1).contiguous()
        t = t.view(n, h * 3 * c, s)
...
class SSFL(nn.Module):
    ...
    def forward(self, t_f, t_s, t_l):
        ...
        part_score = self.part_score(cat_feature.permute(0, 3, 2, 1).contiguous().view(n, -1, s))
